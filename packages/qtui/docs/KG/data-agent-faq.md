...
## 决策（2026-09-20，详见设计文档第 11 节）

| 编号 | 问题 | 决策 | 状态 |
|---|---|---|---|
| D7 | 语义层选型与同步频率 | 优先 OpenMetadata；网关保持元数据源可替换；L1–L3 定制（自定义属性/官方扩展/网关自研），L4 薄 fork 为最后手段；结构日更+事件失效，口径/示例审批即推 | 已定 |
| D8 | 输出契约归属 | 平台管框架与注册，业务管内容与审批；契约不可变版本化，查询携带 `contractId + version` | 已定 |
| D9 | 修复回路位置 | 有人等待 → 单会话；无人等待且单批 ≥50 或需重放 → 外部编排；共用网关工具与契约 | 已定 |
| D10 | 质检规则治理 | 作者/审批人/平台三角色，两级审批；规则版本化、责任人、例外到期、金标集校准 | 已定 |
| D11 | 内核级数据上下文源 | 暂不引入；设 3 个复查触发点（稳定上下文 token/缓存不可接受、合规要求内核级审计、上游开放公共扩展 API） | 已定 |
...
| 2026-09-20 | 待决问题转为决策（D7–D11），语义层确定 OpenMetadata 优先与定制异化策略 |
